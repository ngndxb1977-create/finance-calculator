...
def load_all_vehicle_data(vehicle_file_path):
    ...
    for sheet in xls.sheet_names:
        ...
        try:
            ...
            # Apply dynamic allocation rule: 500 for Attrage & Mirage, 1000 for everything else
...
if not VEHICLE_CATALOG["2025"] and not VEHICLE_CATALOG["2026"]:
    ...
    if st.session_state.view_state == "input":
        ...
        if finance_dp_option:
            ...
            st.caption(f"**Down Payment Breakout Mechanics:** Gross Down Payment: {calculated_downpayment:,.2f} AED | Showroom Reservation Fee Paid Out-of-Pocket: {vehicle_reservation_fee:,.2f} AED | Remaining Financed Base: {dp_financed_base:,.2f} AED Amortized over {dp_months} Months.")
            ...
            for years in tenures:
                months = years * 12
                v_total_interest = finance_amount * bank_rate * years
                v_monthly_emi = (finance_amount + v_total_interest) / months
                
                if months <= dp_months:
                    combined_emi = v_monthly_emi + dp_monthly_emi
                    notes = f"Combined Installment Plan for first {months} Months"
                else:
                    combined_emi = v_monthly_emi
                    notes = f"Drops down cleanly to asset-only EMI after Month {dp_months}"

                emi_results.append({
                    "Term": f"{years} Years ({months} Mos)",
                    "Vehicle Loan EMI (A)": f"{v_monthly_emi:,.2f} AED",
                    "DP Finance EMI (B)": f"{dp_monthly_emi:,.2f} AED" if months <= dp_months or (dp_months == 3 and years == 1) else "0.00 AED (Settled)",
                    "Combined Monthly Pay (A + B)": f"{combined_emi:,.2f} AED",
                    "Schedule Timeline Outlook": notes if months > dp_months else "Combined structure active throughout"
                })
            st.table(pd.DataFrame(emi_results))
        else:
            st.header("2. Standard Vehicle EMI Breakdown")
            tenures = [1, 2, 3, 4, 5]
            emi_results = []
