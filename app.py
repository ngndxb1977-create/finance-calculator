...
if not VEHICLE_CATALOG["2025"] and not VEHICLE_CATALOG["2026"]:
   ...
   if st.session_state.view_state == "input":
       ...
       # Down Payment Loan Table (Shows all three options if feature is toggled)
       if finance_dp_option:
           st.markdown("<br>", unsafe_allow_html=True)
           st.subheader("🔵 Down Payment Loan Financing (Comparative Matrix)")
           ...
           dp_scenarios = [
               {"months": 3, "roi": 0.0000},
               {"months": 12, "roi": 0.0525},
               {"months": 24, "roi": 0.0630}
           ]
           
           dp_summary_results = []
           for s in dp_scenarios:
               dp_months = s["months"]
               dp_interest_rate = s["roi"]
           ...
           st.caption("ℹ️ *Down Payment loan plans run independently from the vehicle asset loan lifecycle.*")
       ...
       with col_out1:
           if finance_dp_option:
               ...
               st.write(f"**Remaining Down Payment Balance:** {max(0.0, calculated_downpayment - v_data['reservation_fee']):,.2f} AED (Financed via Selected Loan Plan)")
           else:
